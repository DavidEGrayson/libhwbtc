...
TEST_CASE("error class basic properties")
{
    ...
    SECTION("can be moved without getting copied")
    {
        ...
        libhwbtc::error error(error_create("hi"));
        libhwbtc_error * p = error.pointer_get();
        REQUIRE(p);

        // Move constructor.
        libhwbtc::error error2 = std::move(error);
...
TEST_CASE("error_create", "[error_create]")
{
    ...
    {
        libhwbtc::error error(error_create("Error1 %d.", 123));
        REQUIRE(error);
    }
...
TEST_CASE("error_add", "[error_add]")
{
    ...
    {
        libhwbtc::error error(error_add(NULL, "hi"));
        REQUIRE(error.message() == "hi");
    }

    SECTION("preserves the message and error codes of errors passed to it")
    {
        libhwbtc::error error(error_add(error_add_code(error_create("hi1"), 7), "hi2"));
...
TEST_CASE("error_add_code", "[error_add_code]")
{
    SECTION("works with NULL")
    {
        libhwbtc::error error(error_add_code(NULL, 4));
        CHECK(error.message() == "");
    ...
    SECTION("preserves the message and error codes of errors passed to it")
    {
        libhwbtc::error error(error_add_code(error_add_code(error_create("hi1"), 7), 9));
...
{
    libhwbtc::error error(&error_no_memory);
    ...
    {
        REQUIRE(error.has_code(LIBHWBTC_ERROR_MEMORY));
    }
}

TEST_CASE("libhwbtc_error_copy")
{
    libhwbtc_error * error = error_create("X.");
    error = error_add(error, "Y.");
    error = error_add_code(error, 7);
    error = error_add_code(error, 8);
    libhwbtc::error error1(error);

    // Test the C++ copy constructor and libhwbtc_error_copy at the same time.
    libhwbtc::error error2 = error1;
...
TEST_CASE("error tests are skipped")
{
    std::cerr << "Skipping unit tests for errors.\n";
}
